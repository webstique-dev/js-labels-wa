...
export default function Products() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      notify.error(err.response?.data?.message || 'Error fetching products');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteProduct = async (id, name) => {
    const isConfirmed = await confirm({
      ...
      message: `Are you sure you want to soft-delete product "${name}"?`,
    ...
    <div className="space-y-6 pb-12">
      <div className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Manage label products, unit pricing, and usage cycle defaults</p>
        </div>
        <div className="relative min-w-[220px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-300 rounded-xl text-xs text-slate-800"
          />
        </div>
      </div>
          ...
          {filteredProducts.map((p) => (
            <div key={p._id} className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs space-y-3 flex flex-col justify-between">
              <div>
                ...
                <h3 className="font-extrabold text-slate-900 text-sm mt-3">{p.name}</h3>
                <p className="text-xs text-slate-500">Category: {p.category || 'General Labels'}</p>
              </div>

              <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                <span className="text-xs text-slate-400 font-semibold">Unit Price</span>
                <span className="text-lg font-black text-slate-900">₹{p.unitPrice}</span>
              </div>
            </div>
          ))}
        </div>
