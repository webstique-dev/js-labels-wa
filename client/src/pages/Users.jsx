...
export default function Users() {
  ...
  return (
    <div className="space-y-6 pb-12 font-sans">
      ...
      {/* Filter & Search Bar wrapped in CollapsibleFilterCard */}
      {(() => {
        const activeCount = (roleFilter ? 1 : 0) + (statusFilter ? 1 : 0) + (searchQuery ? 1 : 0);
                ...
                <AccordionCard
                  ...
                  title={
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-9 h-9 rounded-xl bg-slate-900 text-white flex items-center justify-center font-semibold text-xs shadow-2xs shrink-0">
                        {initials}
                      </div>
                      <div className="min-w-0 flex-1">
                        <span className="font-semibold text-slate-900 text-sm flex items-center gap-1.5 truncate">
                          {u.name}
                          {isSelf && <span className="text-[10px] font-normal text-slate-400 shrink-0">(You)</span>}
                        </span>
                        <p className="text-xs text-slate-500 font-normal truncate">{u.email}</p>
                      </div>
                    </div>
                  }
                  headerExtra={
                    <span className={`px-2 py-0.5 border text-[10px] font-medium rounded-md uppercase shrink-0 ${getRoleBadgeClass(u.role)}`}>
                      {u.role.replace('_', ' ')}
                    </span>
                  }
                >
                  <div className="grid grid-cols-2 gap-3 text-xs pt-1">
                    <div>
                      <span className="text-slate-400 font-medium text-[11px] block">Status</span>
                      <span className={`px-2 py-0.5 border text-[10px] font-semibold rounded-md uppercase inline-block mt-0.5 ${
                        u.status === 'active' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-600 border-slate-200'
                      }`}>
                        {u.status}
                      </span>
                    ...
                    <div>
                      <span className="text-slate-400 font-medium text-[11px] block">Phone / Mobile</span>
                      <a href={`tel:${u.phone}`} onClick={(e) => e.stopPropagation()} className="font-semibold text-slate-800 hover:text-blue-600 truncate block">
                        {u.phone || 'N/A'}
                      </a>
                    </div>
                    <div>
                      <span className="text-slate-400 font-medium text-[11px] block">Joined Date</span>
                      <span className="font-medium text-slate-700">
                        {u.createdAt ? new Date(u.createdAt).toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' }) : 'N/A'}
                      </span>
                    </div>
                    <div>
                      <span className="text-slate-400 font-medium text-[11px] block">Last Login</span>
                      <span className="font-medium text-slate-700">
                        {u.lastLogin ? new Date(u.lastLogin).toLocaleDateString('en-IN', { month: 'short', day: 'numeric', year: 'numeric' }) : 'Never'}
                      </span>
                    </div>
                  </div>
                  ...
                  {!isSelf && (
                    <div className="pt-3 border-t border-slate-200/80 flex items-center justify-end gap-2">
                      <button
                        ...
                        className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-semibold transition cursor-pointer flex items-center gap-1"
                      >
                        <Edit size={13} />
                        <span>Edit Role</span>
                      </button>
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 bg-blue-50 text-blue-700 hover:bg-blue-100 rounded-xl text-xs font-semibold transition cursor-pointer flex items-center gap-1"
                      >
                        <Key size={13} />
                        <span>Password</span>
                      </button>
                      ...
                      {u.status === 'active' ? (
                        <button
                          ...
                          className="p-1.5 text-amber-700 bg-amber-50 hover:bg-amber-100 rounded-xl transition cursor-pointer"
                          title="Deactivate"
                        >
                          <UserX size={15} />
                        </button>
                        ...
                        <button
                          ...
                          className="p-1.5 text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-xl transition cursor-pointer"
                          title="Activate"
                        >
                          <UserCheck size={15} />
                        </button>
                      ...
                      {currentRole === 'super_admin' && (
                        <button
                          ...
                          className="p-1.5 text-rose-600 bg-rose-50 hover:bg-rose-100 rounded-xl transition cursor-pointer"
                          title="Move to Trash"
                        >
                          <Trash2 size={15} />
                        </button>
