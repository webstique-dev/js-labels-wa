...
import { Trash2, RotateCcw, Check, User } from 'lucide-react';
...
export default function Trash() {
  ...
  const fetchTrashItems = async (resource) => {
    ...
    try {
      ...
      notify.error(err.response?.data?.message || 'Failed to fetch trashed items');
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  const getItemDisplayName = (item) => {
    ...
    if (activeTab === 'leads') return `${item.name}${item.company ? ` (${item.company})` : ''}`;
    if (activeTab === 'customers') return `${item.name}${item.company ? ` (${item.company})` : ''}`;
    if (activeTab === 'products') return item.name;
    if (activeTab === 'users') return `${item.name} (${item.email})`;
    if (activeTab === 'followups') return `Follow-up #${item._id?.slice(-6)} (${item.relatedType})`;
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        {loading ? (
          <div className="p-12 text-center">
            ...
            <p className="text-sm text-slate-500 mt-3 font-medium">Loading trashed records...</p>
          </div>
        ) : items.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <h3 className="text-base font-bold text-slate-800">No Trashed Items</h3>
            <p className="text-sm text-slate-500 mt-1">
              There are no deleted records in the <span className="font-semibold">{TABS.find(t => t.key === activeTab)?.label}</span> bin.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {items.map((item) => {
                  ...
                  const deletedByUser = item.deletedBy?.name || item.deletedBy?.email || 'Unknown User';
                    ...
                    <tr key={item._id} className="hover:bg-slate-50/60 transition">
                      ...
                      <td className="py-3.5 px-4 text-right">
                        <button
                          ...
                          className="px-3.5 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs transition shadow-xs flex items-center gap-1.5 ml-auto"
                        >
                          <RotateCcw size={14} />
                          <span>Restore</span>
                        </button>
