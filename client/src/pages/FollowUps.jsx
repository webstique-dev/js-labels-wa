...
export default function FollowUps() {
  ...
  return (
    <div className="space-y-6 pb-12 font-sans">
      ...
      {/* Filter & Search Controls Bar wrapped in CollapsibleFilterCard */}
      {(() => {
      ...
      {/* Customer Follow-ups Data Table & Accordion Cards */}
      {loading ? (
                  ...
                  title={
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        <h4 className="font-semibold text-slate-900 text-sm truncate">{cust.displayName}</h4>
                        <p className="text-slate-500 text-xs font-normal truncate">{cust.displayCompany}</p>
                      </div>
                    </div>
                  }
                  headerExtra={
                    <span className={`px-2 py-0.5 border text-[10px] font-bold rounded-md uppercase shrink-0 ${statusBadgeClass}`}>
                      {statusLabel}
                    </span>
                  }
                >
                  <div className="grid grid-cols-2 gap-3 text-xs pt-1">
                    <div>
                      ...
                      <a href={`tel:${cust.displayPhone}`} onClick={(e) => e.stopPropagation()} className="font-semibold text-slate-800 hover:text-blue-600 truncate block">
                        {cust.displayPhone}
                      </a>
                    </div>
                    <div>
                      <span className="text-slate-400 font-medium text-[11px] block">Follow-up Date</span>
                      <span className="font-semibold text-slate-900 block">{cust.formattedDate}</span>
                      <span className={`text-[10px] font-semibold ${cust.followupStatus === 'overdue' ? 'text-rose-600' : 'text-slate-500'}`}>
                        {cust.daysText}
                      </span>
                    </div>
                    <div>
                      <span className="text-slate-400 font-medium text-[11px] block">Sales Executive</span>
                      <span className="font-medium text-slate-700">{cust.execName}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 font-medium text-[11px] block">Reorder Probability</span>
                      <span className={`font-bold ${getProbabilityTextColorClass(cust.reorderProb)}`}>
                        {cust.reorderProb}%
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="pt-3 border-t border-slate-200/80 flex items-center justify-between gap-2">
                    ...
                    {cust.displayPhone && (
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          initiatePhoneCall(cust.displayPhone);
                        }}
