...
export default function Dashboard() {
  ...
  const handleOpenActivitiesModal = async () => {
    ...
    try {
      ...
      if (period && period !== 'all_time') params.append('period', period);
      if (assignedTo && assignedTo !== 'all') params.append('assignedTo', assignedTo);
      if (source && source !== 'all') params.append('source', source);
      params.append('limit', '100');

      const res = await api.get(`/dashboard/activity-feed?${params.toString()}`);
      setAllActivities(res.data || []);
    } catch (err) {
      console.error('Error fetching all activities modal feed:', err);
      notify.error('Failed to load full activity history');
    } finally {
      setModalLoading(false);
    }
  };

  const filteredModalActivities = allActivities.filter((act) => {
    const desc = (act.description || '').toLowerCase();
    const userName = (act.createdBy?.name || '').toLowerCase();
    const matchesSearch = desc.includes(modalSearch.toLowerCase()) || userName.includes(modalSearch.toLowerCase());
  ...
  useEffect(() => {
    if (isManagerOrAdmin) {
      api.get('/users?role=caller')
        .then((res) => setCallers(Array.isArray(res.data) ? res.data.filter(u => u.role === 'caller') : []))
        .catch((err) => console.warn('Non-fatal error fetching callers for filter:', err));
    }
  }, [isManagerOrAdmin]);

  // Fetch dashboard metrics with query parameters
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      const [sumRes, funRes, trendRes, actRes, altRes] = await Promise.all([
        ...
        api.get(`/dashboard/funnel${queryString}`),
        api.get(`/dashboard/conversion-trend${queryString}`),
        api.get(`/dashboard/activity-feed${queryString ? queryString + '&limit=5' : '?limit=5'}`),
        api.get(`/dashboard/alerts${queryString}`)
      ]);
  ...
  }, [period, assignedTo, source, notify]);
  ...
  const isFilterActive = period !== 'this_month' || assignedTo !== 'all' || source !== 'all';
    ...
    <div className="space-y-6 pb-12 font-sans">
      ...
      {showActivitiesModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[85vh] flex flex-col shadow-2xl border border-slate-200 overflow-hidden animate-fadeIn font-sans">
            
            {/* Modal Header */}
            <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 shrink-0">
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 rounded-xl bg-red-50 text-red-600 flex items-center justify-center font-bold shadow-2xs">
                  <ActivityIcon size={18} />
                </div>
                <div>
                  <h3 className="font-bold text-slate-900 text-base">All System Activities</h3>
                  <p className="text-xs text-slate-500 font-normal">Complete timeline of calls, WhatsApp messages, reorder updates, and notes</p>
                </div>
              </div>
              <button
            ...
            <div className="p-4 bg-white border-b border-slate-100 flex flex-col sm:flex-row items-center gap-3 shrink-0">
              <div className="relative flex-1 w-full">
                <input
                  type="text"
                  value={modalSearch}
                  onChange={(e) => setModalSearch(e.target.value)}
                  placeholder="Search activities by description or executive..."
                  className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-red-500"
                />
              ...
              <select
                ...
                className="w-full sm:w-auto px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-700 cursor-pointer focus:outline-none"
              >
                <option value="all">All Activity Types</option>
                <option value="call">Calls</option>
                <option value="whatsapp">WhatsApp</option>
                <option value="note">Notes</option>
                <option value="reorder_date">Reorder Dates</option>
            ...
            <div className="p-5 overflow-y-auto flex-1 space-y-3">
              {modalLoading ? (
                ...
                filteredModalActivities.map((act, idx) => {
                  const iconInfo = getActivityIcon(act, idx);
                  return (
                    <div key={act._id || idx} className="p-3.5 bg-slate-50/70 hover:bg-slate-100/80 border border-slate-100 rounded-xl flex items-start justify-between gap-3 transition">
                      <div className="flex items-start gap-3 min-w-0">
                        <div className={`w-9 h-9 rounded-xl ${iconInfo.bg} flex items-center justify-center shrink-0 border mt-0.5`}>
                          {iconInfo.icon}
                        </div>
                        <div className="min-w-0">
                          <p className="font-semibold text-slate-900 text-xs leading-relaxed">{act.description}</p>
                          <div className="flex flex-wrap items-center gap-2 text-[11px] text-slate-400 mt-1 font-normal">
                            <span>By {act.createdBy?.name || 'System User'}</span>
                            {act.type && (
                              <span className="px-2 py-0.5 bg-white border border-slate-200 rounded-md text-[10px] uppercase font-semibold text-slate-600">
                                {act.type}
                              </span>
                            )}
                          </div>
                      ...
                      <div className="text-right text-[11px] text-slate-400 font-medium shrink-0 leading-tight">
                        <div>{act.createdAt ? new Date(act.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : ''}</div>
                        <div className="text-[10px] text-slate-400 mt-0.5">{act.createdAt ? new Date(act.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}</div>
                      </div>
                    </div>
                  );
                })
                ...
                <div className="py-12 text-center text-slate-400 text-xs font-normal">
                  No activity records match the selected search/filter.
                </div>
              )}
            </div>
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between shrink-0">
              <span className="text-xs text-slate-500 font-medium">
                Showing {filteredModalActivities.length} of {allActivities.length} activities
              </span>
              <button
                onClick={() => setShowActivitiesModal(false)}
